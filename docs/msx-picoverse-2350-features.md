...
- **microSD ROM Execution up to 2 MB**: ROM files on the microSD card are streamed into the cartridge's external 8 MB PSRAM (QMI CS1, 52.5 MHz QPI) and executed directly from there. The leading window of the ROM is also mirrored into 256 KB of internal SRAM for low-latency mapper hot-loop access, so larger ROMs run with the same performance as flash-resident titles. The size cap per microSD ROM is 2 MB.
