...
- **ESP-01 WiFi Support For Sunrise IDE Modes**: The LoadROM tool accepts `-w` together with `-s1`, `-m1`, `-s2`, or `-m2`, adding an embedded ESP8266P system ROM and a memory-mapped UART backend for the PicoVerse 2350 ESP-01 header. This is currently a LoadROM-only feature and is documented separately in the WiFi guide.
